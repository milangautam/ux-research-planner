...
A [Claude Code](https://claude.ai/code) skill that guides researchers through building complete, executable UX research plans — module by module, with a validation checkpoint at every stage.

## What it does

The skill acts as a senior UX research strategist. Given a product and a problem statement, it walks you through the full planning process and produces documents that can be handed directly to a team and executed without ambiguity.

**Primary framework:** OMSEP — Objectives → Methodology → Scope → Essentials → Protocol

It also supports alternative frameworks and handles partial requests (e.g., "I just need screener questions" or "write me a discussion guide") without forcing you through the full flow.
...
| 2 | **Objectives** | Research goals + questions, tagged by type and mapped to methods |
| 3 | **Methodology** | Finalized methods with rationale, pros, cons, and which questions they answer |
| 4 | **Scope** | What is and isn't being tested — flows, screens, features, user groups |
...
| 7 | **Recruitment Plan** | User group profiles, participant counts, recruitment methods, edge cases |
...
Load the skill in Claude Code and use any of these prompts:
...
**Constraint-aware** — all recommendations (methods, tools, participant counts, incentives) adapt to your budget, timeline, team size, geography, and available tools.
